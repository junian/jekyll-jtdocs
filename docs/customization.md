...
jekyll-jtdocs supports two color schemes: light (default), and dark.
...
In jekyll-jtdocs version `0.4.2`, we changed the default syntax highlighting theme for the `light` color scheme to have higher contrast. Users who want to use the old highlighting need to explicitly opt-in with the deprecated `legacy_light` color scheme. We removed this in `v0.11.0` as part of a broader accessibility pass on the theme.
...
Available variables are listed in the [\_variables.scss](https://github.com/junian/jekyll-jtdocs/tree/main/_sass/support/_variables.scss) file.
...
This is most commonly-used to define [custom callout colors]({{ site.baseurl }}{% link docs/configuration.md %}#callouts). For example,
...
jekyll-jtdocs provides the following custom includes files:
...
Any content added to this file will appear at the bottom left of the page below the site's navigation. By default an attribution to jekyll-jtdocs is displayed which reads, `This site uses jekyll-jtdocs, a documentation theme for Jekyll.`.
...
jekyll-jtdocs uses Jekyll's powerful [layouts](https://jekyllrb.com/docs/layouts/) and [includes](https://jekyllrb.com/docs/includes/) features to generate and compose various elements of the site. Jekyll users and developers can extend or replace existing layouts and includes to customize the entire site layout.

### Default layout and includable components

The `default` layout is inherited by most of the "out-of-the-box" pages provided by jekyll-jtdocs. It composes various re-usable components of the site, including the sidebar, navbar, footer, breadcrumbs, and various imports. Most users who create new pages or layouts will inherit from `default`.
...
- `_includes/head.html` is the entire `<head>` tag for the site; this imports stylesheets, various JavaScript files (ex: analytics, mermaid, search, and jekyll-jtdocs code), and SEO / meta information.
...
Similarly, users and developers can create other alternative layouts using jekyll-jtdocs' reusable includable components.
...
By default, Jekyll (and its default theme `minima`) provide the `about`, `home`, `page`, and `post` layouts. In jekyll-jtdocs, we override all of these layouts with the `default` layout. Each of those layouts is simply:
