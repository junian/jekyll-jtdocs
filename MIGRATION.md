...
- Mermaid support: first-class support for [Mermaid](https://mermaid.js.org/) - a JavaScript-based diagram and charting tool supported by GitHub - has been added to the theme. **This feature is opt-in.** See the new doc subsections in [Configuration]({{ site.baseurl }}{% link docs/configuration.md %}#mermaid-diagrams) and [Code]({{ site.baseurl }}{% link docs/ui-components/code/index.md %}#mermaid-diagram-code-blocks) for more.
...
We've added several custom `_includes` to provide users with more customization options for different site elements. We've also added a section to [Configuration]({{ site.baseurl }}{% link docs/customization.md %}#override-includes) to outline these.
...
For more, see [Custom layouts and includes]({{ site.baseurl }}{% link docs/customization.md %}#custom-layouts-and-includes). PR: [#1058].
...
In addition to customizing the search placeholder, we've also added the ability to provide custom content to the search index. for more, see [Custom content for search index]({{ site.baseurl }}{% link docs/search.md %}#custom-content-for-search-index). PR: [#1068].
...
- Links wrap. PR: [#905]
- Callouts: a new component similar to alerts or banners. See [UI Components - Callouts]({{ site.baseurl }}{% link docs/ui-components/callouts.md %}). PR: [#466]
